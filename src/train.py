...
def train_wrapper(config_name: str = "config"):
    """Wrapper to dynamically set config name"""
    ...
    def train(cfg: DictConfig):
        ...
        loss_type = cfg.model.get("loss_type", "MSE")
        use_resnet = cfg.model.get("use_resnet", False)
        perceptual_weight = cfg.model.get("perceptual_weight", 0.0)
        ...
        logger = TensorBoardLogger(
            ...
            name=f"{cfg.logger.name}_{cfg.data.dataset_name}",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train VAE with configurable config file"
    )
    parser.add_argument(
        "--config-name",
        type=str,
        default="config",
        help="Name of the config file (without .yaml extension). Default: config",
    )
    args, unknown = parser.parse_known_args()

    # Get the train function with the specified config
    train_fn = train_wrapper(config_name=args.config_name)
    # Call it to trigger Hydra
    train_fn()
