...
def find_worst_reconstructions(model, loader, device, k=16):
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            mse_per_image = F.mse_loss(recons, imgs, reduction='none').view(imgs.size(0), -1).mean(dim=1)
...
def main():
    ...
    grid_worst = torchvision.utils.make_grid(combo_worst, nrow=8, normalize=False) # 8 per row -> 2 rows orig, 2 rows recon
